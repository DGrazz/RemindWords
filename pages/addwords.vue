  ...
  <main class="w-[70%] mx-auto">
    ...
    <section class="w-full h-[70vh] flex flex-col justify-center items-center gap-8">
      ...
      <article
        ...
        <h3 class="font-medium text-xl flex gap-3 items-center">Added words
          <button @click="toggleEditWords" class="p-1 rounded-md transition duration-200"
            :class="editWords ? 'bg-green-400 hover:bg-green-500' : 'bg-red-400 hover:bg-red-500'">
            <transition name="fade" mode="out-in">
              <EditIcon v-if="!editWords" class="size-4" key="edit-icon" />
              <CheckIcon v-else class="size-4" key="check-icon" />
            </transition>
          </button>
        </h3>
...
export default {
  ...
  methods: {
    addWord() {
      ...
      if (this.word) {
        if (/^[a-zA-Z]+$/.test(this.word) && this.word.length >= 2) {
          ...
          this.toast.error("Please add a valid word", {
            position: 'top-center',
            timeout: 2000,
          });
        }
      } else {
        this.toast.error("Please add a word", {
          position: 'top-center',
          timeout: 2000,
        });
      }
    ...
    toggleEditWords() {
      ...
      if (this.editWords) {
        this.toast.info("Editing words");
      } else {
        this.toast.success("Done editing words");
      }
    },
    removeWord(index) {
      this.words.splice(index, 1);
      this.toast.error("Word removed");
    },
  },
  created() {
    const store = useStore();
